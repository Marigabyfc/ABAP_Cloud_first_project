CLASS zmgfc_string_processing DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ZMGFC_STRING_PROCESSING IMPLEMENTATION.
...
*    DATA(text) = |USER Date:{ the_date Date = USER }|.
    ...
    DATA(text2) = |User Format{ my_number NUMBER = USER      }|.
    DATA(text3) = |Sign Right { my_number SIGN = RIGHT       }|.
    DATA(text4) = |Scientific { my_number STYLE = SCIENTIFIC }|.
